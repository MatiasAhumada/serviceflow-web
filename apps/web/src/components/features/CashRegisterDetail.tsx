        ...
        <div className="flex items-center justify-between">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              <h2 className="text-2xl font-bold text-foreground">{cashRegister.name}</h2>
              <Badge variant={cashRegister.status === "open" ? "success" : "outline"}>{cashRegister.status === "open" ? "Abierta" : "Cerrada"}</Badge>
            </div>
            {cashRegister.user && <p className="text-sm text-muted-foreground">Cajero asignado: {cashRegister.user.name}</p>}
          </div>
          <Button variant={cashRegister.status === "open" ? "destructive" : "default"} onClick={cashRegister.status === "open" ? onClose : onOpen}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card variant="stats">
            <CardContent className="p-4">
              ...
              <p className="text-2xl font-bold text-foreground">{formatters.currency(cashRegister.currentBalance)}</p>
            </CardContent>
          </Card>
          <Card variant="stats">
            <CardContent className="p-4">
              <p className="text-sm text-muted-foreground">Ingresos</p>
              <p className="text-2xl font-bold text-green-600">{formatters.currency(stats.totalIncome)}</p>
            </CardContent>
          </Card>
          <Card variant="stats">
            <CardContent className="p-4">
              <p className="text-sm text-muted-foreground">Egresos</p>
              <p className="text-2xl font-bold text-red-600">{formatters.currency(stats.totalExpense)}</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="orders" className="w-full">
          ...
          <TabsContent value="orders" className="space-y-4">
            <GenericTable
              ...
              emptyMessage={cashRegister.status === "open" ? "No hay órdenes pendientes" : "Abrí la caja para ver las órdenes pendientes"}
      ...
      >
        <div className="space-y-4">
          ...
          <div className="text-center">
            ...
            <p className="text-3xl font-bold text-foreground">{formatters.currency(selectedOrder?.amount || 0)}</p>
          </div>
