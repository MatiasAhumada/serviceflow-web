...
export default function SuppliersPage() {
  ...
  const { suppliers, stats, isLoading, createSupplier, updateSupplier, deleteSupplier } = useSuppliers({
    ...
    <>
      ...
      <header className="bg-background border-b border-border px-4 sm:px-6 py-4 pb-7">
        <div className="flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-xl sm:text-2xl font-bold text-[#111827] dark:text-white">Proveedores</h1>
            <p className="text-xs sm:text-sm text-[#10B981] font-medium">Gestiona tus proveedores</p>
          </div>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <GenericModal
          ...
          mode={modalMode}
          title={modalMode === "create" ? "Nuevo Proveedor" : modalMode === "update" ? "Editar Proveedor" : modalMode === "delete" ? "Eliminar Proveedor" : "Detalles del Proveedor"}
        >
          {modalMode === "delete" ? (
            <p>¿Está seguro que desea eliminar a <strong>{selectedSupplier?.name}</strong>?</p>
